...
pub fn draw_left_panel(app: &mut PaintApp, ctx: &egui::Context) {
    ...
    if !app.show_minimal_ui {
        let panel_width = if app.workspace_layout.left_panel_collapsed { 22.0 } else { app.workspace_layout.left_panel_width };
        let panel_min_w = if app.workspace_layout.left_panel_collapsed { 22.0 } else { 180.0 };
        let panel_max_w = if app.workspace_layout.left_panel_collapsed { 22.0 } else { 500.0 };
        ...
        side_panel.show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                let collapse_label = if app.workspace_layout.left_panel_collapsed { "▶" } else { "◀" };
                let resp = ui.add_sized(
                    egui::vec2(14.0, 14.0),
                    egui::Button::new(egui::RichText::new(collapse_label).size(9.0)),
                ).on_hover_text(if app.workspace_layout.left_panel_collapsed { "Expand Left Panel" } else { "Collapse Left Panel" });
                if resp.clicked() {
            ...
            if !app.workspace_layout.left_panel_collapsed {
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        ui.vertical(|ui| {
                            // ── TOOLS / BRUSH PRESETS ──
                            if app.workspace_layout.is_panel_at(PanelKind::ToolsAndPresets, PanelLocation::Left) {
                            let _tp_resp = panel_section(ui, "TOOLS / BRUSH PRESETS", |ui| {
                                draw_tools_and_presets_content(app, ui, ctx);
                            });
                            _tp_resp.context_menu(|ui| crate::ui::panel_location_menu(ui, PanelKind::ToolsAndPresets, app));
                            } // is_panel_at ToolsAndPresets

                            // ── BRUSH SETTINGS ──
                            if app.workspace_layout.is_panel_at(PanelKind::BrushSettings, PanelLocation::Left) {
                            let _bs_resp = panel_section(ui, "BRUSH SETTINGS", |ui| {
                                draw_brush_settings_content(app, ui, ctx);
                            });
                            _bs_resp.context_menu(|ui| crate::ui::panel_location_menu(ui, PanelKind::BrushSettings, app));
                            } // is_panel_at BrushSettings

                            // ── TOOL OPTIONS ──
                            if app.workspace_layout.is_panel_at(PanelKind::ToolOptions, PanelLocation::Left) {
                            let _to_resp = panel_section(ui, "TOOL OPTIONS", |ui| {
                                draw_tool_options_content(app, ui, ctx);
                            });
                            _to_resp.context_menu(|ui| crate::ui::panel_location_menu(ui, PanelKind::ToolOptions, app));
...
pub(crate) fn draw_tools_and_presets_content(app: &mut PaintApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    egui::Grid::new("tools_grid")
        ...
        .show(ui, |ui| {
            ...
            let corners = [
                ...
                r.center_top(), r.center_bottom(), r.left_center(), r.right_center(),
            ];
            for &c in &corners {
            ...
            let center = btn_resp.rect.center();
            ui.painter().text(center, egui::Align2::CENTER_CENTER, "T", egui::FontId::new(14.0, egui::FontFamily::Proportional), icon_color);
    ...
    egui::Grid::new("vector_tools_grid")
        ...
        .show(ui, |ui| {
            ...
            for &(tool_id, label, tooltip) in &vec_tools {
                ...
                let btn = egui::Button::new(egui::RichText::new(label).size(12.0)).selected(is_active);
                ...
                for i in 0..16 {
                    if i < num_presets {
                        ...
                        let response = ui.dnd_drag_source(id, i, |ui| {
                            ...
                            let font_size = if name_len > 8 { 5.5 } else if name_len > 6 { 6.5 } else { 8.0 };
                            let name_font = egui::FontId::proportional(font_size);
                            let display_name = if name_len > 9 { format!("{}..", &preset_name[0..7]) } else { preset_name.clone() };
                            let name_pos = rect.min + egui::vec2(3.0, 9.0);
                            ui.painter().text(name_pos, egui::Align2::LEFT_BOTTOM, display_name, name_font, text_color);

                            let icon_font = egui::FontId::proportional(8.0);
                            let icon_pos = rect.max - egui::vec2(3.0, 3.0);
                            ui.painter().text(icon_pos, egui::Align2::RIGHT_BOTTOM, type_tag, icon_font, text_color);
                            btn_response
                        });
                        ...
                        let (rect, btn_response) = ui.allocate_exact_size(egui::vec2(34.0, 30.0), egui::Sense::click());
                        let bg_color = if btn_response.hovered() { egui::Color32::from_gray(245) } else { egui::Color32::WHITE };
                        ui.painter().rect_filled(rect, 2.0, bg_color);
                        ui.painter().rect_stroke(rect, 1.0, egui::Stroke::new(1.0, egui::Color32::from_gray(225)));
                        ui.painter().text(rect.center(), egui::Align2::CENTER_CENTER, "+", egui::FontId::proportional(14.0), egui::Color32::GRAY);

                        let mut show_creation_menu = false;
                        if btn_response.clicked() { show_creation_menu = true; }
                        btn_response.context_menu(|ui| {
                            ui.label("Create New Brush:");
                            if ui.button("Pencil").clicked() { app.create_preset(PresetIcon::Pencil); ui.close_menu(); }
                            if ui.button("Ink Pen").clicked() { app.create_preset(PresetIcon::InkPen); ui.close_menu(); }
                            if ui.button("Paint Brush").clicked() { app.create_preset(PresetIcon::PaintBrush); ui.close_menu(); }
                            if ui.button("Smudge").clicked() { app.create_preset(PresetIcon::Smudge); ui.close_menu(); }
                            if ui.button("Eraser").clicked() { app.create_preset(PresetIcon::Eraser); ui.close_menu(); }
                            ui.separator();
                            ui.label("Import Brush Preset:");
                            ui.horizontal(|ui| { ui.label("Path:"); ui.text_edit_singleline(&mut app.brush_import_path); });
                            if ui.button("Load .artybrush").clicked() {
                                ...
                                match crate::brush_io::load_artybrush(path, &mut app.brush_textures) {
                                    Ok(mut new_preset) => {
                                        ...
                                        if new_preset.is_eraser { PaintApp::set_constant(&mut brush, BrushSetting::Eraser, 1.0); }
                            ...
                            if ui.button("⚡ Extract & Import .sut").clicked() {
                                ...
                                match crate::brush_io::extract_sut_texture(path) {
                                    Ok((gray_bytes, w, h)) => {
                                        ...
                                        for y in 0..h.min(256) { for x in 0..w.min(256) { final_bytes[(y * 256 + x) as usize] = gray_bytes[(y * w + x) as usize]; } }
                                        let name = path.file_stem().and_then(|s| s.to_str()).unwrap_or("SUT Brush").to_string();
                                        app.brush_textures.push(crate::app::BrushTexture { name: format!("[sut] {}", name), width: 256, height: 256, pixels: final_bytes });
                                        let texture_id = (app.brush_textures.len() - 1) as u32;
                                        app.preset_id_counter += 1;
                                        let new_preset = crate::app::BrushPreset {
                                            id: app.preset_id_counter, name: path.file_stem().and_then(|s| s.to_str()).unwrap_or("SUT Brush").to_string(),
                                            icon: PresetIcon::PaintBrush, radius_log: 2.0, opacity: 1.0, hardness: 0.8,
                                            min_size_fraction: 0.2, color_blending: 0.0, dilution: 0.0, is_eraser: false,
                                            texture_id, texture_scale: 1.0, bristle_id: 0,
                                            stabilizer_level: StabilizerLevel::default(), stabilizer_mode: StabilizerMode::SpringMassDamper,
                                            spacing: 2.0, density: 1.0,
                                        };
                                        let mut brush = Brush::new();
                                        PaintApp::set_constant(&mut brush, BrushSetting::Radius, new_preset.radius_log);
                                        PaintApp::set_constant(&mut brush, BrushSetting::Opaque, new_preset.opacity);
                        ...
                        if show_creation_menu { ui.ctx().memory_mut(|mem| mem.open_popup(btn_response.id.with("context_menu"))); }
                    }
                    if i % 4 == 3 { ui.end_row(); }
                }
            });
    ...
    if let Some(idx) = app.renaming_preset_index {
        ...
        ui.horizontal(|ui| {
            ...
            if res.lost_focus() || ui.button("OK").clicked() {
                if !app.rename_input.trim().is_empty() { app.presets[idx].name = app.rename_input.trim().to_string(); }
                app.renaming_preset_index = None;
            }
            if ui.button("✕").on_hover_text("Cancel rename").clicked() { app.renaming_preset_index = None; }
        });
    }
...
pub(crate) fn draw_brush_settings_content(app: &mut PaintApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    if matches!(app.active_tool, ToolId::Brush | ToolId::Eraser) {
        // --- 1. BRUSH STROKE PREVIEW ---
        let preview_height = 56.0;
        let (preview_resp, preview_painter) = ui.allocate_painter(egui::vec2(ui.available_width(), preview_height), egui::Sense::hover());
            ...
            for xi in 0..cols {
                if (xi + yi) % 2 == 1 {
                    preview_painter.rect_filled(
                        egui::Rect::from_min_size(egui::pos2(pr.min.x + xi as f32 * cell, pr.min.y + yi as f32 * cell), egui::Vec2::splat(cell)),
                        0.0, egui::Color32::from_gray(220),
                    );
                }
            }
        }
        preview_painter.rect_stroke(pr, 0.0, egui::Stroke::new(1.0, egui::Color32::from_gray(160)));

        // Draw stylized brush stroke (pressure-sensitive magenta stroke)
        let num_dabs = 50;
        let max_r = (2.0 + app.brush_radius_log.max(0.0) * 3.0).min(14.0).max(1.5);
        ...
        for i in 0..num_dabs {
            ...
            let alpha = ((1.0 - app.brush_hardness * (1.0 - pressure) * 0.4) * stroke_opacity * 255.0) as u8;
        ...
        for i in 0..num_dabs - 1 {
            ...
            let p1 = egui::pos2(pr.min.x + 8.0 + t1 * (pr.width() - 16.0), pr.center().y + (t1 - 0.5).sin() * 10.0);
            let p2 = egui::pos2(pr.min.x + 8.0 + t2 * (pr.width() - 16.0), pr.center().y + (t2 - 0.5).sin() * 10.0);
            preview_painter.line_segment([p1, p2], egui::Stroke::new(0.5, egui::Color32::from_rgba_unmultiplied(180, 30, 70, (stroke_opacity * 255.0) as u8)));
        }
        ...
        ui.horizontal(|ui| {
            ...
            let tip_types: [(&str, &str, bool); 4] = [
                ...
                ("●", "Hard Round", app.brush_hardness >= 0.6 && app.brush_hardness < 0.9),
                ("⬟", "Marker", app.brush_hardness >= 0.9 && app.brush_hardness < 1.0),
            ...
            for (idx, &(_icon, _name, is_sel)) in tip_types.iter().enumerate() {
                ...
                if is_selected { selected_tip_idx = idx; }
            }
            ...
            for (idx, &(icon, _name, _)) in tip_types.iter().enumerate() {
                ...
                if ui.add_sized([24.0, 22.0], btn).on_hover_text(tip_names[idx]).clicked() {
                    match idx {
        ...
        ui.horizontal(|ui| {
            ...
            if ui.add(egui::DragValue::new(&mut size_display).speed(0.1).suffix("").clamp_range(0.1..=150.0)).changed() {
        ...
        ui.horizontal(|ui| {
            ui.label("Min Size");
            ui.add(egui::Slider::new(&mut app.brush_min_size_fraction, 0.0..=1.0).show_value(false));
            if ui.add(egui::DragValue::new(&mut app.brush_min_size_fraction).speed(0.01).suffix("%")).changed() {
        ...
        ui.horizontal(|ui| {
            ...
            if ui.add(egui::DragValue::new(&mut app.brush_density).speed(0.01).suffix("%")).changed() {
        ...
        ui.horizontal(|ui| {
            ...
            ui.add_enabled(false, egui::Slider::new(&mut min_density, 0.0..=100.0).show_value(false));
            ui.add_enabled(false, egui::DragValue::new(&mut min_density).speed(1.0).suffix("%"));
        });
        ...
        egui::collapsing_header::CollapsingState::load_with_default_open(
            ...
            ui.horizontal(|ui| {
                ui.label("Scale:");
                let mut scale = 100.0;
                ui.add_enabled(false, egui::Slider::new(&mut scale, 10.0..=500.0).show_value(false));
                ui.add_enabled(false, egui::DragValue::new(&mut scale).speed(1.0).suffix("%"));
            });
        ...
        let texture_name = app.brush_textures.get(app.brush_texture_id as usize)
            .map(|t| t.name.as_str()).unwrap_or("None");
        let has_texture = app.brush_texture_id > 0;
        egui::collapsing_header::CollapsingState::load_with_default_open(
            ui.ctx(), ui.next_auto_id(), has_texture
        ).show_header(ui, |ui| {
            ui.label(format!("[ {} ]", if has_texture { texture_name } else { "No Texture" }));
        }).body(|ui| {
            ui.horizontal(|ui| {
                ui.label("Texture:");
                ...
                let res = egui::ComboBox::from_id_source("brush_texture_combo_sai")
                    ...
                    .show_ui(ui, |ui| {
                        ...
                        for (idx, tex) in app.brush_textures.iter().enumerate() {
                            if ui.selectable_value(&mut selected_tex, idx as u32, &tex.name).clicked() { changed = true; }
                        }
                        changed
                    });
                if res.inner.unwrap_or(false) { app.brush_texture_id = selected_tex; app.brush_settings_dirty = true; }
            });
            if has_texture {
                ui.horizontal(|ui| {
                    ui.label("Intensity");
                ...
                ui.horizontal(|ui| {
                    ui.label("Scale");
                    ui.add(egui::Slider::new(&mut app.brush_texture_scale, 0.1..=10.0).show_value(false));
                ...
                ui.horizontal(|ui| {
                    ui.label("Intensity");
                    let mut dummy = 95.0;
                    ui.add_enabled(false, egui::Slider::new(&mut dummy, 0.0..=100.0).show_value(false));
                    ui.add_enabled(false, egui::DragValue::new(&mut dummy).speed(1.0));
                });
                ui.horizontal(|ui| {
                    ui.label("Scale");
                    let mut dummy_s = 100.0;
                    ui.add_enabled(false, egui::Slider::new(&mut dummy_s, 10.0..=500.0).show_value(false));
            ...
            if has_texture {
                ...
                ui.add_enabled(false, egui::Checkbox::new(&mut false, "Invert for Transparency"));
            }
        });
        ...
        egui::collapsing_header::CollapsingState::load_with_default_open(
            ...
            ui.horizontal(|ui| {
                ui.label("Blending");
                if ui.add(egui::Slider::new(&mut app.brush_color_blending, 0.0..=1.0).show_value(false)).changed() {
                    app.brush_settings_dirty = true;
                }
                if ui.add(egui::DragValue::new(&mut app.brush_color_blending).speed(0.01)).changed() {
                    app.brush_settings_dirty = true;
                }
            });
            ui.horizontal(|ui| {
                ui.label("Persistence");
                if ui.add(egui::Slider::new(&mut app.brush_dilution, 0.0..=1.0).show_value(false)).changed() {
                    app.brush_settings_dirty = true;
                }
                if ui.add(egui::DragValue::new(&mut app.brush_dilution).speed(0.01)).changed() {
                    app.brush_settings_dirty = true;
                }
            });
            ui.horizontal(|ui| {
                ui.label("Opacity");
                if ui.add(egui::Slider::new(&mut app.brush_opacity, 0.0..=1.0).show_value(false)).changed() {
                    app.brush_settings_dirty = true;
                }
        ...
        egui::collapsing_header::CollapsingState::load_with_default_open(
            ...
            ui.horizontal(|ui| {
                ui.label("Sharpness");
                if ui.add(egui::Slider::new(&mut app.brush_hardness, 0.0..=1.0).show_value(false)).changed() {
                    app.brush_settings_dirty = true;
                }
                if ui.add(egui::DragValue::new(&mut app.brush_hardness).speed(0.01)).changed() {
                    app.brush_settings_dirty = true;
                }
            });
            ...
            ui.horizontal(|ui| {
                ...
                ui.add_enabled(false, egui::Slider::new(&mut amplify, 0.0..=100.0).show_value(false));
                ui.add_enabled(false, egui::DragValue::new(&mut amplify).speed(1.0));
            });
            ui.horizontal(|ui| {
                ui.label("Spacing");
                if ui.add(egui::Slider::new(&mut app.brush_spacing, 0.5..=10.0).show_value(false)).changed() {
                    app.brush_settings_dirty = true;
                }
                if ui.add(egui::DragValue::new(&mut app.brush_spacing).speed(0.1)).changed() {
                    app.brush_settings_dirty = true;
                }
            });
            ...
            ui.horizontal(|ui| {
                ui.label("Bristle ID");
                if ui.add(egui::DragValue::new(&mut app.brush_bristle_id).speed(1).clamp_range(0..=5)).changed() {
                    app.brush_settings_dirty = true;
                }
            });
            ui.add_enabled(false, egui::Checkbox::new(&mut false, "Ver1 Prs Spec"));
            let mut anti_ripple = matches!(app.stabilizer.level, StabilizerLevel::Level(_) | StabilizerLevel::SLevel(_));
            if ui.checkbox(&mut anti_ripple, "Anti-Ripple").changed() {
                app.stabilizer.set_level(if anti_ripple { StabilizerLevel::Level(8) } else { StabilizerLevel::Off });
                app.brush_settings_dirty = true;
            }
        });
        ...
        egui::collapsing_header::CollapsingState::load_with_default_open(
                ...
                let response = egui::ComboBox::from_id_source("sai_stabilizer_level")
                    ...
                    .show_ui(ui, |ui| {
                        ...
                        if ui.selectable_label(matches!(current_level, StabilizerLevel::Off), "Off").clicked() {
                            app.stabilizer.set_level(StabilizerLevel::Off); selected = true;
                        }
                        for val in 1..=15 {
                            let is_sel = matches!(current_level, StabilizerLevel::Level(v) if v == val);
                            if ui.selectable_label(is_sel, format!("Level {}", val)).clicked() {
                                app.stabilizer.set_level(StabilizerLevel::Level(val)); selected = true;
                            }
                        }
                        for val in 1..=5 {
                            let is_sel = matches!(current_level, StabilizerLevel::SLevel(v) if v == val);
                            if ui.selectable_label(is_sel, format!("S-{}", val)).clicked() {
                                app.stabilizer.set_level(StabilizerLevel::SLevel(val)); selected = true;
                            }
                        }
                        selected
                    });
                if response.inner.unwrap_or(false) { ctx.request_repaint(); app.brush_settings_dirty = true; }
            });
            ui.horizontal(|ui| {
                ui.label("Mode");
                ...
                let response = egui::ComboBox::from_id_source("sai_stabilizer_mode")
                    ...
                    .show_ui(ui, |ui| {
                        ...
                        if ui.selectable_label(current_mode == StabilizerMode::Ema, "EMA").clicked() {
                            app.stabilizer.mode = StabilizerMode::Ema; selected = true;
                        }
                        if ui.selectable_label(current_mode == StabilizerMode::SpringMassDamper, "Spring Physics").clicked() {
                            app.stabilizer.mode = StabilizerMode::SpringMassDamper; selected = true;
                        }
                        selected
                    });
                if response.inner.unwrap_or(false) { ctx.request_repaint(); app.brush_settings_dirty = true; }
            ...
            ui.horizontal(|ui| {
                ...
                ui.add(egui::DragValue::new(&mut ps_hard_soft_size).speed(1.0).suffix("%"));
            });
            ...
            ui.horizontal(|ui| {
                ...
                ui.add(egui::DragValue::new(&mut ps_blending).speed(1.0).suffix("%"));
            });
        });
        ...
        // --- 9. SYMMETRY SECTION ---
        let symmetry_on = app.symmetry_mode != crate::app::SymmetryMode::None;
        egui::collapsing_header::CollapsingState::load_with_default_open(
            ui.ctx(), ui.next_auto_id(), symmetry_on
        ).show_header(ui, |ui| {
            ui.label("Symmetry");
        }).body(|ui| {
            ui.horizontal(|ui| {
                ...
                egui::ComboBox::from_id_source("symmetry_mode_sai")
                    ...
                    .width(80.0)
                    .show_ui(ui, |ui| {
                        if ui.selectable_label(matches!(app.symmetry_mode, crate::app::SymmetryMode::None), "Off").clicked() { app.symmetry_mode = crate::app::SymmetryMode::None; }
                        if ui.selectable_label(matches!(app.symmetry_mode, crate::app::SymmetryMode::Horizontal), "Horizontal").clicked() { app.symmetry_mode = crate::app::SymmetryMode::Horizontal; }
                        if ui.selectable_label(matches!(app.symmetry_mode, crate::app::SymmetryMode::Vertical), "Vertical").clicked() { app.symmetry_mode = crate::app::SymmetryMode::Vertical; }
                        if ui.selectable_label(matches!(app.symmetry_mode, crate::app::SymmetryMode::Radial), "Radial").clicked() { app.symmetry_mode = crate::app::SymmetryMode::Radial; }
                    });
            });
            ...
            ui.horizontal(|ui| {
                ui.label("Center X:");
                ui.add(egui::DragValue::new(&mut app.symmetry_center.x).clamp_range(0.0..=4096.0).speed(1.0));
            });
            ui.horizontal(|ui| {
                ui.label("Center Y:");
                ui.add(egui::DragValue::new(&mut app.symmetry_center.y).clamp_range(0.0..=4096.0).speed(1.0));
            });
            ui.horizontal(|ui| {
        ...
        // --- 10. ADVANCED / DEBUG SECTION ---
        egui::collapsing_header::CollapsingState::load_with_default_open(
            ui.ctx(), ui.next_auto_id(), false
        ).show_header(ui, |ui| {
            ui.label("Advanced / Debug");
        }).body(|ui| {
            ui.horizontal(|ui| {
                ui.label("Pressure");
                ui.add(egui::Slider::new(&mut app.pressure_curve, 0.25..=2.50).show_value(false));
                if ui.add(egui::DragValue::new(&mut app.pressure_curve).speed(0.05)).changed() {
                }
                ui.label("curve");
            });
            ui.horizontal(|ui| {
                ...
                if ui.add(egui::DragValue::new(&mut app.pressure_min).speed(0.005)).changed() {
            ...
            let (bar_response, bar_painter) = ui.allocate_painter(egui::vec2(bar_width, 8.0), egui::Sense::hover());
            let bar_rect = bar_response.rect;
            bar_painter.rect_filled(bar_rect, 0.0, egui::Color32::from_gray(60));
            let filled = egui::Rect::from_min_max(
                bar_rect.min,
                egui::pos2(bar_rect.min.x + bar_rect.width() * pressure_frac, bar_rect.max.y),
            );
...
pub(crate) fn draw_tool_options_content(app: &mut PaintApp, ui: &mut egui::Ui, _ctx: &egui::Context) {
    match app.active_tool {
        ...
        ToolId::Fill => {
            ...
            match app.fill_options.detection_mode {
                crate::tools::fill::FillDetectionMode::ColorDifference => {
                    ui.horizontal(|ui| { ui.label("Color Diff:"); ui.add(egui::Slider::new(&mut app.fill_options.tolerance, 0..=255)); });
                }
                crate::tools::fill::FillDetectionMode::TransparencyFuzzy => {
                    ui.horizontal(|ui| { ui.label("Transp Diff:"); ui.add(egui::Slider::new(&mut app.fill_options.transp_diff, 0..=255)); });
                }
            ...
            ui.horizontal(|ui| { ui.label("Expand:"); ui.add(egui::Slider::new(&mut app.fill_options.expand_px, 0..=10)); });
        ...
        ToolId::RectSelect | ToolId::EllipseSelect | ToolId::Lasso | ToolId::PolygonLasso => {
            ...
            ui.horizontal(|ui| { ui.label("Feather:"); ui.add(egui::Slider::new(&mut app.selection_feather, 0.0..=100.0)); });
        }
        ToolId::MagicWand => {
            ui.horizontal(|ui| {
                ui.label("Mode:");
        ...
        ToolId::Gradient => {
            ui.horizontal(|ui| {
                ...
                if gm != app.gradient_mode { app.gradient_mode = gm; }
            });
            ui.horizontal(|ui| {
                ui.label("Type:");
                ...
                if gt != app.gradient_type { app.gradient_type = gt; }
            });
        }
        ToolId::ColorPicker => {
